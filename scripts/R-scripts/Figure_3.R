...
thermus_protein_protein=read.csv("/home/suu13/projects/antisense/Avoidance_Git/tables/TableS5_Thermus_SSU_profile.csv",sep="\t")
...
plot_avoidance_regions = function() {
  ...
  image(t(matrix((thermus_protein_protein$Avoidance.P.Vals < 0.001),nrow=1,ncol=nrow(thermus_protein_protein))),axes=T,col=c("white","red3"),xaxt ="n",yaxt="n" )
  ...
  image(t(matrix((thermus_protein_protein$Protein_Bind_Computed),nrow=1,ncol=nrow(thermus_protein_protein))),axes=T,col=c("white","royalblue4"),xaxt ="n",yaxt="n" )
